...
class EMRLoader(object):
    ...
    def create_bucket_on_s3(self, bucket_name):
        ...
        try:
            ...
            logger.info(
                "Bucket does not exist: {error}. I will create it!".format(error=e))
...
def main():
    ...
    logger.info(
        "*******************************************+**********************************************************")
    logger.info(
        "Check if bucket exists otherwise create it and upload files to S3.")
    emr_loader.create_bucket_on_s3(
        bucket_name=config_emr.get("script_bucket_name"))
    emr_loader.upload_to_s3("bootstrap_actions.sh", bucket_name=config_emr.get("script_bucket_name"),
                            key_name="bootstrap_actions.sh")
    emr_loader.upload_to_s3("pyspark_quick_setup.sh", bucket_name=config_emr.get("script_bucket_name"),
    ...
    while True:
        ...
        job_state_reason = job_response.get("Cluster").get(
            "Status").get("StateChangeReason").get("Message")
    ...
    if step:
        ...
        add_step_response = emr_loader.add_step(
            emr_response.get("JobFlowId"), master_dns)
        ...
        while True:
            ...
            if len(list_steps_response.get("Steps")) == len(
                ...
                logger.info(emr_client.list_steps(
                    ClusterId=emr_response.get("JobFlowId")))
    else:
